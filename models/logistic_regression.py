...
from interpretibility_utils import extract_linear_features
...
for lemma in lemmas:
    print(f"[LR] Processing lemma: {lemma}")
    ...
    if df_word["sense"].nunique() < 2:
        print(f"[LR] Skipping {lemma} (not enough senses)")
        continue
    ...
    extract_linear_features(model, vectorizer, lemma, "LR")
...
print("LR results saved to:", output_path)
